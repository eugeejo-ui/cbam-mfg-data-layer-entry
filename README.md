...
- CBAM 1차 적용 대상 중견 기업의 정보기술부문 투자액 중앙값: **14.3억 원** (CBAM 비대상 중견 기업 33.3억 원의 절반 이하, 비대상에 약한 연결 후보 6곳 포함)
...
![CBAM 적용 단계와 배출 데이터 연결 구조](figures/fig2_cbam_data_flow.png)
...
| 중견 · 비대상 (약한 연결 후보 6곳 포함) | 33.3억 | 1.3% | 5.1% | 10.4% |

- IT 예산 증가율 = watsonx.data 온라인 버전 연간 비용 ÷ 정보기술부문 투자액
- CBAM 대응은 필수 지출 성격으로, "IT 예산의 증가 폭"으로 해석함
- 최소 구성: 소형 엔진·업무시간 가동·기본 지원비 상시 과금 / 상시 운영 구성: 중형 엔진·24시간 가동
...
| 경로 | 내용 | 국내 보관 | 가격 공개 |
|---|---|---|---|
| A. 온라인 버전(SaaS) | IBM 운영 | 제한 (IBM Cloud·AWS 공개 리전 목록에 서울 없음, Azure 리전 미확인) | 공개 |
...
| 5 | 그래프·도식 작성 | `05_figures.py`, `05b_diagram.py` |
| 6 | 문서 수치 대조 (149개 항목 일치 확인) | `06_verify_numbers.py` |
...
- CBAM 대상 품목 EU 수출 기업 다수가 중소·중견 기업이라는 분석(KOTRA)이 있어, 측정 제외 구간의 부담이 더 클 가능성 있음
...
python src\05b_diagram.py
python src\06_verify_numbers.py
```

- `01b`, `01c`, `02b`, `03b`는 점검용 코드
- `06_verify_numbers.py`는 문서 기재 수치와 결과 파일을 대조하는 검증용 코드
